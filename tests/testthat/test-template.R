...
#    `functionName` is a function's name in your module (e.g., `Boo2019Event1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/fstewart/AppData/Local/Temp/Rtmp2lwGAX/SpaDES/modules/Boo2019/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("Boo2019")
  path <- list(modulePath = "C:/Users/fstewart/AppData/Local/Temp/Rtmp2lwGAX/SpaDES/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    Boo2019 = list(.saveInitialTime = NA)
  )
  ...
  if (exists("Boo2019Event1", envir = .GlobalEnv)) {
    simOutput <- Boo2019Event1(mySim)
  } else {
  ...
  if (exists("Boo2019Event2", envir = .GlobalEnv)) {
    simOutput <- Boo2019Event2(mySim)
  } else {
