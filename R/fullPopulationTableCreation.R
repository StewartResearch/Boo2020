fullPopulationTableCreation <- function(dataPathGDrive = "https://drive.google.com/open?id=1avEapiUArQx28paSz28WlWrT69fZ7hf1", 
  ...
  PopulationTable <- data.table::rbindlist(lapply(X = herdScenario, FUN = function(herdScen){
  ...
  if (typeOfFile == "rds"){
    saveRDS(object = PopulationTable, file = paste0(dataName, ".rds"))
  } else {
    if (typeOfFile == "csv")
      write.csv(x = PopulationTable, file = paste0(dataName, ".csv"))
  }
