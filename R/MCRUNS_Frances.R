MCRUNS_F<-function (Area, Regime, IND = 50, CF = 1, Density = 0.04) 
{
  InitPop <- function(K) {
  ...
  K <- round(Area/100)*(Density * 0.5)[1]
  P0 <- InitPop(K)
  print(P0)
  ...
  Stage1 <- Caribou_F(K = K, p50s = pYoung1, hoof = fp1, Pop = P0)
  ...
  AS2 <- tmp2$EndForest
  fp2 <- IND
  Stage2 <- Caribou_F(K = K, p50s = pYoung2, hoof = fp2, Pop = Stage1$EndPop)
  ...
  n <- length(pYoung2)
  Stage3 <- Caribou_F(K = K, p50s = pYoung3, hoof = fp3, Pop = Stage2$EndPop)
